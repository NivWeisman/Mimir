...
- ✅ `textDocument/didSave` — registered via the `save` option on `text_document_sync` (so the editor sends save notifications). Handler logs the save and nudges `schedule_elaborate` so the slang sidecar's view of the compilation unit reflects what's now on disk. v1 deferrals / limitations: no save-triggered re-parse (incremental sync already kept state fresh — `did_change` is authoritative); no save-time diagnostics flush; `save.includeText: false` (the buffer is already in our rope, no need for the editor to re-send it); no `willSave` / `willSaveWaitUntil`. No-op when slang isn't configured.
...
- ✅ `textDocument/signatureHelp` — slang-first with tree-sitter fallback. When slang is configured the sidecar resolves the callable under the cursor and returns its declared signature (stub returns empty today, full slang implementation is a future slice). Tree-sitter fallback: finds the enclosing call via `tf_call` / `system_tf_call` / `text_macro_usage` nodes, looks up the callee in the same-file and workspace symbol indices, and emits one `SignatureInformation` with parameter offsets for active-parameter highlighting. Trigger characters: `(` and `,`. Method calls (`.method(...)`) are skipped by the tree-sitter path — receiver type resolution requires slang.
...
- ⬜ Multi-hop chained member access in hover (`a.b.c.method()`) — v1 receiver resolution covers one hop only; deeper chains fall through to bare-identifier lookup.
- ⬜ `textDocument/codeLens`

### Navigation

- ✅ `textDocument/definition` — tree-sitter index, same-file + workspace-wide (open docs and `.mimir.toml` filelist, with the filelist hydration transitively following `` `include `` chains). Routes through slang's semantic resolver (scope-aware, hierarchical-name-aware) when `MIMIR_SLANG_PATH` is configured; falls back to the syntax index both on transport error *and* on an empty slang result, so positions slang can't resolve (e.g. a type name inside `IDENT#(T)`) still get a workspace-index answer. Slang resolves variable / port / parameter / class-field references, hierarchical paths (`u_dut.fsm.state`), `obj.member`, subroutine calls (`f(x)`, `obj.method()`), type references in declarations (`my_t x;` → typedef/class), module/interface instantiations (`apb_master u_dut(...)` → the module), base-class references in `extends` clauses, and macro `` `define `` sites (`` `MY_MACRO `` → the `` `define `` line, including cross-file defines pulled in via the filelist).
- ✅ `textDocument/declaration` — slang-first with tree-sitter workspace-index fallback, mirroring `textDocument/definition`. Slang's `definition` RPC resolves to declaration sites (the identifier token of the declaring construct), so it is reused here — no dedicated sidecar RPC is needed. The tree-sitter fallback looks up the symbol name in the same-file index and workspace index, identical to `goto_definition`. v1 deferral: **prototype-vs-body distinction** — `extern function` / `pure virtual` prototypes and their external `function_body_declaration` counterparts are both indexed as `SymbolKind::Function`; the resolver returns whichever appears first in the same file (or the workspace-index entry on a miss). A future v2 slice will add `is_prototype` tracking to `Symbol` and a dedicated slang RPC so declaration jumps to the prototype while definition jumps to the body.
