...
- ⬜ Keyword / system-task hover help — static descriptions for SV keywords (`always_ff`, `initial`, `assert`, …) and system tasks (`$display`, `$fatal`, `$cast`, …). Needs a new `KEYWORD_DOCS` table in [`crates/mimir-syntax/src/keywords.rs`](./crates/mimir-syntax/src/keywords.rs).
