...
- ✅ `textDocument/inlayHint` — tree-sitter-only. Finds all call sites in the editor's visible viewport, looks up each callee in the same-file and workspace symbol indices, and places `param: type` ghost-text labels before each argument. Function/task calls show typed labels (`a: int`); macro callsites (`` `uvm_fatal("ID", "MSG") `` and friends) show bare param names (`ID`, `MSG`) — macro args carry no SV type. Whole-line macro callsites are preserved past the preprocessor under the 0.3.1 grammar's compiler-directive allowlist, so AST-backed features see them as `text_macro_usage` nodes. `super.X(...)` and `this.X(...)` method calls are resolved through the AST: the enclosing `class_declaration` gives the current class, its `extends` clause the parent, and the inlay handler walks the inheritance chain through `Symbol::parent_class_name` until it finds a matching method (including `extern virtual` prototypes — UVM's `run_phase`/`build_phase` style declarations now produce real param hints). Other method calls (`obj.method(...)`) and bare `new(...)` constructor expressions are still skipped — they need slang's semantic resolver to know the receiver's type. Calls with more arguments than declared parameters are silently skipped (avoids wrong labels for variadic-style patterns).
