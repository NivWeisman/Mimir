...
- ✅ `textDocument/inlayHint` — tree-sitter-only. Finds all call sites in the editor's visible viewport, looks up each callee in the same-file and workspace symbol indices, and places `param: type` ghost-text labels before each argument. Function/task calls show typed labels (`a: int`); macro calls show bare names (`x`). Method calls (`.method(...)`) are silently skipped — receiver type resolution requires slang. Calls with more arguments than declared parameters are silently skipped (avoids wrong labels for variadic-style patterns).
