...
- ✅ `textDocument/definition` — tree-sitter index, same-file + workspace-wide (open docs and `.mimir.toml` filelist, with the filelist hydration transitively following `` `include `` chains). Routes through slang's semantic resolver (scope-aware, hierarchical-name-aware) when `MIMIR_SLANG_PATH` is configured; an empty slang result is returned as-is ("no declaration found") rather than falling back to the name-based syntax index, because a fuzzy name match across all files is less accurate than slang's authoritative "nothing here". Transport errors fall back to the syntax index so a crashed sidecar doesn't leave the feature completely dark. Slang resolves variable / port / parameter / class-field references, hierarchical paths (`u_dut.fsm.state`), `obj.member`, subroutine calls (`f(x)`, `obj.method()`), type references in declarations (`my_t x;` → typedef/class), module/interface instantiations (`apb_master u_dut(...)` → the module), base-class references in `extends` clauses, and macro `` `define `` sites (`` `MY_MACRO `` → the `` `define `` line, including cross-file defines pulled in via the filelist).
